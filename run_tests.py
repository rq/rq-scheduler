...
def main():
    ...
    env = "ONLY_RUN_FAST_TESTS=1" if "-f" not in sys.argv else ""
    pipe = "rg" if Popen(['which', 'rg'], stdout=PIPE).wait() == 0 else "cat"

    #run tests and gather output
    p = Popen("%s /usr/bin/env python -m unittest discover -v -s tests %s" % (env, " ".join(sys.argv[1:])), shell=True, stdout=PIPE, stderr=STDOUT)
    exit_code = p.wait()
    (out, _) = p.communicate()
